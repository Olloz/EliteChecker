...
values = sorted([(member["uuid"], sum(member["expHistory"].values())) for member in data["guild"]["members"] if sum(member["expHistory"].values()) > 450000 and currentTime - member["joined"] > 691200000], key = lambda x: x[1], reverse=True)

def to_username(uuid):
    return session.get(f"https://api.hypixel.net/player?key={hypixeldata['hypixelKey']}&uuid={uuid}").json()["player"][
        "displayname"]
